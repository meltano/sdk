...
def test_mapped_stream(
    ...
    if "properties" in schema_message.schema:
        schema_properties = schema_message.schema["properties"].keys()
    else:
        schema_properties = schema_message.schema.keys()

    assert schema_properties == output_fields
