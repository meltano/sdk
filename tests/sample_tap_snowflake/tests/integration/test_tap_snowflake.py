...
class TestTypeMapping(unittest.TestCase):
    ...
    def get_dt_metadata_for_column(self, col_name):
        ...
        return next(
            md
            for md in self.dt_metadata
            if md["breadcrumb"] == ("properties", col_name)
        )["metadata"]
    ...
    def test_discover_catalog_with_single_table(self):
        ...
        tap_stream_ids = [s.tap_stream_id for s in catalog.streams]
        self.assertCountEqual(
            tap_stream_ids, ["ANALYTICS_DB_TEST-TAP_SNOWFLAKE_TEST-EMPTY_TABLE_2"]
        )
    ...
    def test_discover_catalog_with_not_existing_table(self):
        ...
        with self.assertRaises(snowflake.connector.errors.ProgrammingError):
            test_utils.discover_catalog(
                self.snowflake_conn,
                {
                    "tables": f"{SCHEMA_NAME}.empty_table_2,{SCHEMA_NAME}.not_existing_table"
                },
            )
    ...
    def test_discover_catalog_with_view(self):
        ...
        tap_stream_ids = [s.tap_stream_id for s in catalog.streams]
        self.assertCountEqual(
            tap_stream_ids, ["ANALYTICS_DB_TEST-TAP_SNOWFLAKE_TEST-EMPTY_VIEW_1"]
        )
    ...
    def test_decimal(self):
        ...
        self.assertEqual(
            self.get_dt_metadata_for_column("C_DECIMAL"),
            {"selected-by-default": True, "sql-datatype": "number"},
        )

    def test_decimal_with_defined_scale_and_precision(self):
        self.assertEqual(
            self.dt_schema.properties["C_DECIMAL_2"],
            Schema(["null", "number"], inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_DECIMAL_2"),
            {"selected-by-default": True, "sql-datatype": "number"},
        )

    def test_smallint(self):
        self.assertEqual(
            self.dt_schema.properties["C_SMALLINT"],
            Schema(["null", "number"], inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_SMALLINT"),
            {"selected-by-default": True, "sql-datatype": "number"},
        )

    def test_int(self):
        self.assertEqual(
            self.dt_schema.properties["C_INT"],
            Schema(["null", "number"], inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_INT"),
            {"selected-by-default": True, "sql-datatype": "number"},
        )

    def test_bigint(self):
        self.assertEqual(
            self.dt_schema.properties["C_BIGINT"],
            Schema(["null", "number"], inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_BIGINT"),
            {"selected-by-default": True, "sql-datatype": "number"},
        )

    def test_float(self):
        self.assertEqual(
            self.dt_schema.properties["C_FLOAT"],
            Schema(["null", "number"], inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_FLOAT"),
            {"selected-by-default": True, "sql-datatype": "float"},
        )

    def test_double(self):
        self.assertEqual(
            self.dt_schema.properties["C_DOUBLE"],
            Schema(["null", "number"], inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_DOUBLE"),
            {"selected-by-default": True, "sql-datatype": "float"},
        )

    def test_date(self):
        self.assertEqual(
            self.dt_schema.properties["C_DATE"],
            Schema(["null", "string"], format="date-time", inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_DATE"),
    ...
    def test_time(self):
        ...
        self.assertEqual(
            self.get_dt_metadata_for_column("C_TIME"),
            {"selected-by-default": True, "sql-datatype": "time"},
        )

    def test_binary(self):
        self.assertEqual(
            self.dt_schema.properties["C_BINARY"],
            Schema(["null", "string"], format="binary", inclusion="available"),
        )
        self.assertEqual(
            self.get_dt_metadata_for_column("C_BINARY"),
            {"selected-by-default": True, "sql-datatype": "binary"},
        )
    ...
    def test_varbinary(self):
        ...
        self.assertEqual(
            self.get_dt_metadata_for_column("C_VARBINARY"),
            {"selected-by-default": True, "sql-datatype": "binary"},
        )
    ...
    def test_row_to_singer_record(self):
        ...
        with self.snowflake_conn.open_connection() as open_conn:
            with open_conn.cursor() as cur:
                ...
                self.assertEquals(
                    json.loads(formatted_record)["stream"], "TEST_TYPE_MAPPING"
                )
                self.assertEquals(
                    json.loads(formatted_record)["record"],
                    {
                        "C_PK": 1,
                        "C_DECIMAL": 12345,
